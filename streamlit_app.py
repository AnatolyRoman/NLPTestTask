...
st.markdown(f'''
    <h1 style="text-align: left; font-family: 'Gill Sans'; color: #FF69B4">Вывод:</h1>
...
Исходя из сравнения метрик, модель 2 оказалась более успешной в классификации эмоций на текстовых данных по сравнению с моделью 1. Она показала более высокую точность, метрики precision, recall, F1-меру и MCC. Это говорит о том, что модель 2 лучше справляется с определением эмоций в тексте и имеет более сбалансированную производительность.

Развитие данного эксперимента:
Для дальнейшего развития данного эксперимента можно рассмотреть следующие аспекты:

Дополнительная настройка модели: Попробовать различные архитектуры нейронных сетей, гиперпараметры, функции активации и оптимизаторы для оптимизации производительности моделей.

Использование предобученных эмбеддингов: Попробовать использовать предварительно обученные эмбеддинги слов, такие как Word2Vec или GloVe, чтобы улучшить представление слов и повысить производительность модели.

Расширение обучающего набора данных: Собрать или добавить больше разнообразных текстовых данных для обучения моделей. Это может помочь моделям лучше обобщать и улучшить их производительность.

Ансамбль моделей: Рассмотреть использование ансамбля моделей, где несколько моделей комбинируются для принятия окончательного решения. Это может помочь улучшить общую производительность и стабильность предсказаний.

Интерпретация результатов: Исследовать методы интерпретации результатов моделей, такие как анализ важности признаков или визуализация активации, чтобы лучше понять, какие слова или фразы влияют на классификацию эмоций.

Эти дополнительные шаги помогут дальше улучшить производительность моделей в задаче классификации эмоций на текстовых данных.
